...
function removeFirstOccurrences(str, value) {
  ...
  const posNext = pos + length;
  const str1 = str.slice(0, pos);
  const str2 = str.slice(posNext);
  const str3 = str1 + str2;
  return pos >= 0 ? str3 : str;
}
...
function isPalindrome(str) {
  return (
    str.split('').reverse().join('').toUpperCase().replaceAll(/[^\w]/g, '') ===
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < arrStr.length; i += 1) {
    if (alphabet.includes(arrStr[i])) {
      arrStr[i] = encode[alphabet.indexOf(arrStr[i])];
    }
  }
  return arrStr.join('');
}
